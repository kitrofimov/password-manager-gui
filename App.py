...
class App:
    ...
    def __init__(self, window_title, debug=False):
        ...
        dpg.create_viewport(title=self.window_title,
    ...
    def ask_to_log_in(self):
        if self.ask_master:  # log in
            ...
            dpg.add_button(label='Submit', callback=handlers.log_in.log_in, parent=self.window, user_data={
                'input': input_master,
                'app': self
            })

        else:  # set up the master password
            text = dpg.add_text('Hello! Please, set your MASTER PASSWORD below. (?)',
                                wrap=400, parent=self.window)
            with dpg.tooltip(text):
                dpg.add_text('This is a password, that you are going to use EVERY TIME you log in in this password manager. If you lose it, you are going to lose all your other passwords!',
                                wrap=300)
            ...
            dpg.add_button(label='Submit', callback=handlers.log_in.set_master_password, user_data={
    ...
    def render_contents(self):  # render the main contents of the app
        with dpg.menu_bar(parent=self.window):
            with dpg.menu(label='File'):
                dpg.add_menu_item(label='Import', callback=handlers.menubar.file.import_passwords, user_data={
                    'app': self
                })
                dpg.add_menu_item(label='Export', callback=handlers.menubar.file.export_passwords, user_data={
                    'app': self
                })
                dpg.add_menu_item(label='Change encryption key', callback=handlers.menubar.file.change_encryption_key, user_data={
                    'app': self
                })
            with dpg.menu(label='About'):
                dpg.add_menu_item(label='Help', callback=handlers.menubar.about.help)
                dpg.add_menu_item(label="Path to program's directory", callback=handlers.menubar.about.path, user_data={
                    'app': self
                })
        ...
        with dpg.group(parent=self.window, tag='form'):
            ...
            dpg.add_button(label='Add', callback=handlers.form.add_password, user_data={
                'inputs': {
                    'name': input_name,
            ...
            dpg.add_button(label='Generate password', callback=handlers.form.generate, user_data={
        ...
        with dpg.table(parent=self.window, before='form') as self.table:
            ...
            for row_i in range(len(self.df)):
                ...
                for column_i in range(3):
                    ...
                    if column_i == 0:
                        dpg.add_text(self.df.iloc[row_i][self.df.columns[column_i]], parent=cell)
                    elif column_i == 1:
                        dpg.add_button(label='Show', parent=cell, callback=handlers.table.show_password, user_data={
                            'app': self,
                            'row_i': row_i
                        })
                    else:
                        dpg.add_button(label='X', parent=cell, callback=handlers.table.delete_password, user_data={
                            'app': self,
                            'row_i': row_i
                        })
