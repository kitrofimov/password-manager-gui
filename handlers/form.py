...
def add_password(sender, app_data, user_data):  # add new password
    input_name, input_password = user_data['inputs']['name'], user_data['inputs']['password']
    app = user_data['app']

    input_name_value, input_password_value = dpg.get_value(input_name), dpg.get_value(input_password)
    ...
    if not len(input_name_value):  # if there is nothing in "name" field
        ...
        app.df = pd.concat([
            app.df,
            pd.DataFrame([[input_name_value, Fernet(app.key).encrypt(bytes(input_password_value, 'utf-8')).decode()]],
...
def generate(sender, app_data, user_data):  # generate password
    ...
    def generate_password():  # code to execute if user wants to generate a password
        [dpg.delete_item(item) for item in dpg.get_item_children(popup)[1]]  # delete everything from popup
        dpg.add_text('Number of symbols:', parent=popup)
        slider = dpg.add_slider_int(min_value=8, default_value=16, max_value=32, parent=popup)
        dpg.add_button(label='Generate', parent=popup, callback=lambda x: 
            dpg.set_value(input_password, secrets.token_urlsafe(  # generating password
    ...
    def generate_passphrase():  # code to execute if user wants to generate a passphrase
        ...
        def generate():  # generate a passphrase
            ...
            dpg.set_value(input_password,
                          xp.generate_xkcdpassword(mywords, acrostic="face", delimiter=dpg.get_value(separator_input)))
    ...
    with dpg.window(popup=True) as popup:  # ask user to choice what type of password to generate
