...
):  # called as callback for first-time user to set his master password
...
def log_in(
    sender, app_data, user_data
):  # called as a callback when user tries to log-in
    ...
    if hashlib.md5(dpg.get_value(input_bar).encode()).hexdigest() == hash:
        ...
        with dpg.window(popup=True):
            ...
            dpg.add_button(
                label="OK", callback=lambda x: dpg.delete_item(dpg.get_item_parent(x))
            )
